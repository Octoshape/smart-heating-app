...
/**
 * This is the {@link View} for showing the schedule for each room of the system.
 */
...
public class ScheduleView extends View {
    ...
    private int mNowLineColor = Color.rgb(39, 137, 228);
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mEntryRects != null && mEntryRects.size() > 0 && mEntryClickListener != null) {
                List<EntryRect> reversedEntryRects = new ArrayList<>(mEntryRects);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mEntryRects == null || mRefreshEntries) {
            mRefreshEntries = false;
            if (mEntryRects == null) {
                mEntryRects = new ArrayList<>();
            }
        ...
        for (int dayNumber = mFirstDayOfWeek; dayNumber < mFirstDayOfWeek + 7; dayNumber++) {
            ...
            boolean sameDay = dayNumber == mToday.get(Calendar.DAY_OF_WEEK);
            ...
            // Draw the entries.
            drawEntries(((dayNumber + 6) % 8 + 1), startPixel, canvas);
            ...
            if (sameDay) {
                ...
                canvas.drawLine(startPixel, startY + beforeNow, startPixel + mWidthPerDay, startY + beforeNow, mNowLinePaint);
            }
        ...
        for (int dayNumber = mFirstDayOfWeek; dayNumber < mFirstDayOfWeek + 7; dayNumber++) {
            ...
            boolean sameDay;
            if (dayNumber > 7) {
                sameDay = (dayNumber % 7) == mToday.get(Calendar.DAY_OF_WEEK);
            } else {
                sameDay = dayNumber == mToday.get(Calendar.DAY_OF_WEEK);
            }

            // Draw the day labels.
            canvas.drawText(mDayLabels.get((dayNumber + 6) % 7), startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
    ...
    /**
     * Draw all the entries of a particular day.
     *
     * @param day            The day.
     * @param startFromPixel The left position of the day area. The entries will never go any left from this value.
     * @param canvas         The canvas to draw upon.
     */
    private void drawEntries(int day, float startFromPixel, Canvas canvas) {
    ...
    /**
     * Draw the name of the entry on top of the entry rectangle.
     *
    ...
    /**
     * Refreshes the view and loads the entries again.
     */
    ...
    public interface ScheduleEntryClickListener {
        void onScheduleEntryClick(ScheduleEntry entry, RectF eventRect, ScheduleEntry nextEntry, int hourClicked);

        void onScheduleEntryLongPress(ScheduleEntry entry, RectF eventRect);
    }
    ...
    public interface GetCurrentScheduleListener {
        List<ScheduleEntry> getCurrentSchedule();
    }
